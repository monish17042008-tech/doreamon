      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
        {popularEpisodes.map((ep) => (
          <div
            ...
            className="group bg-white rounded-2xl border border-slate-200/80 overflow-hidden shadow-xs hover:shadow-card transition-all duration-200 cursor-pointer flex flex-col"
          >
            ...
            <div className="relative aspect-video w-full overflow-hidden bg-slate-100">
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="w-12 h-12 rounded-full bg-white/90 group-hover:bg-sky-500 text-slate-800 group-hover:text-white shadow-lg flex items-center justify-center transition-all transform group-hover:scale-110">
                  <Play className="w-5 h-5 ml-0.5 fill-current" />
                </div>
              </div>

              {/* Era Badge */}
              <div className="absolute top-3 left-3 px-2.5 py-0.5 rounded-md bg-white/90 backdrop-blur-sm text-sky-800 text-[11px] font-bold shadow-xs">
                Classic
              </div>

              {/* Duration */}
              <div className="absolute bottom-3 right-3 flex items-center gap-1 px-2 py-0.5 rounded bg-slate-900/80 text-white text-[11px] font-mono">
            ...
            <div className="p-4 flex-1 flex flex-col justify-between">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1.5 text-xs text-slate-500">
                  <span className="flex items-center gap-1 text-sky-600 font-semibold">
                    <Play className="w-3 h-3 fill-sky-600" /> Episode {ep.episodeNumber}
                  </span>
                  <span>•</span>
              ...
              <div className="mt-3 flex flex-wrap gap-1.5 pt-2 border-t border-slate-100">
                {ep.featuredGadgets.map((gadget) => (
                  <span
                    key={gadget}
                    className="px-2 py-0.5 rounded-md bg-sky-50 text-sky-700 text-[11px] font-medium"
                  >
                    {gadget}
                  </span>
                ))}
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full overflow-hidden shadow-2xl border border-slate-200">
            <div className="relative aspect-video w-full bg-slate-900">
              ...
              <button
                ...
                className="absolute top-3 right-3 w-8 h-8 rounded-full bg-slate-900/80 hover:bg-slate-900 text-white flex items-center justify-center"
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="p-6 space-y-4 text-left">
              ...
              <div className="bg-amber-50 rounded-xl p-3 border border-amber-100">
                <h4 className="text-xs font-bold text-amber-800 uppercase tracking-wider flex items-center gap-1.5">
                  <Star className="w-3.5 h-3.5 text-amber-500 fill-amber-500" /> Moral Lesson
                </h4>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedEpisode.featuredGadgets.map((g) => (
                    <span
                      key={g}
                      className="px-2.5 py-1 rounded-lg bg-sky-100 text-sky-800 text-xs font-bold"
                    >
                      {g}
                    </span>
                  ))}
